...
class WatchConnector: NSObject, ObservableObject, WCSessionDelegate {
    ...
    func session(_ session: WCSession, didReceiveMessage message: [String : Any]) {
        ...
        DispatchQueue.main.async {
            ...
            self.manager.make(accX: self.accX, accY: self.accY, accZ: self.accZ, gyrX: self.gyrX, gyrY: self.accY, gyrZ: self.accZ)
        }
    }
...
struct WatchSensorData {
    ...
    mutating func getDataURLs(label: String) -> [URL] {
        ...
        let accelerometerFilepath = tmppath + "\(apd).csv"
        let gyroFilepath = tmppath + "\(apd).csv"
